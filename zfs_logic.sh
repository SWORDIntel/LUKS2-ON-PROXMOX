...
setup_zfs_pool() {
    ...
    if [[ "${CONFIG_VARS[ZFS_NATIVE_ENCRYPTION]:-no}" == "yes" ]]; then
        ...
        if [[ "${CONFIG_VARS[USE_YUBIKEY_FOR_ZFS_KEY]:-no}" == "yes" ]]; then
            ...
            local zfs_pass zfs_pass_confirm
            while true; do
                echo # Newline for cleaner prompt separation
                read -r -s -p "Enter new ZFS passphrase for pool '${CONFIG_VARS[ZFS_POOL_NAME]}': " zfs_pass
                ...
                if [[ "$zfs_pass" == "$zfs_pass_confirm" ]]; then
                    ...
                    show_error "ZFS passphrases do not match. Please try again."
                fi
            done
