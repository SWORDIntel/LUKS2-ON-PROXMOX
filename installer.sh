...
main() {
    ...
    if [[ "$run_from_ram" == true ]]; then
        ...
        if [[ "$no_ram_boot" == true ]]; then
            ...
            if ! _prompt_user_yes_no "You have selected --no-ram-boot. This is a DANGEROUS mode and prevents installing to the boot media. Are you absolutely sure you want to proceed?"; then
                log_debug "--no-ram-boot dangerous operation not confirmed by user. Aborting."
                exit 1
            fi
